/**
 * \file grille.c
 * \brief Ce fichier implémente les fonctions définies dans \b grille.h
 */
...
Tuile retire_derniere_tuile(Grille g)
{
	...
	for(k1=0; k1<6; k1++)
	{
		...
		{
			l--;
			int cm2 = g->tuiles[l]->t->orient % 2;
			i2 = g->tuiles[l]->i;
			j2 = g->tuiles[l]->j;
			
			/*on parcourt cette tuile pour voir si elle contient le point a, b*/
			for(k2=0; k2<6; k2++)
			{
				i3 = k2/(3-cm2);
				j3 = k2%(3-cm2);
...
int nb_cases_recouvertes(int i, int j, Tuile t, Grille g)
{
	int* cases = cases_recouvertes(i, j, t, g);
	int compteur = 0;
	int k;
...
int* cases_recouvertes(int i, int j, Tuile t, Grille g)
{
	...
	if (t->orient % 2 == 0)
	{
		...
		for(k = 0; k < 6; k++)
		{
			if (coordonnees[k] >= n * n)
				cases[indice++] = -1;
			else if (lecture_case_brut(coordonnees[k], g) != N)
